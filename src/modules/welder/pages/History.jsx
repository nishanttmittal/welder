/**
 * History (admin) — review dispatch entries. Same audit policy as the rest of
 * the system: Edit qty/date, Void (qty→0 + reason, kept as a struck record),
 * and Hard Delete (admin password required). Each change is logged against the
 * entry, and every entry shows its own edit-history.
 */
...
export default function History() {
  ...
  const saveEdit = () => {
    ...
    log('EDIT', `${editing.finishedName || editing.productName}: ${editing.qty}→${v}${editDate !== editing.date ? ` · date ${fmtDate(editing.date)}→${fmtDate(editDate)}` : ''}`, 'admin', editing.id)
  ...
  const voidEntry = (d) => {
    ...
    log('VOID', `${d.finishedName || d.productName} × ${d.qty} → 0${reason ? ' · ' + reason : ''}`, 'admin', d.id)
    show('Voided ✓')
  }
  const hardDelete = (d) => {
    ...
    log('DELETE', `${d.finishedName || d.productName} × ${d.qty} (hard delete)`, 'admin', d.id)
            ...
            return (
              <Card key={d.id} className="p-4">
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <Button size="sm" variant="danger" className="flex-1" onClick={() => hardDelete(d)}>Delete</Button>
                </div>
                {hist.length > 0 && (
                  <div className="mt-2">
                    <button onClick={() => setHistoryOf(historyOf === d.id ? null : d.id)} className="text-xs font-bold text-blue-600">{historyOf === d.id ? 'Hide' : `History (${hist.length})`}</button>
                    {historyOf === d.id && (
                      <div className="mt-1.5 space-y-1">
                        {hist.map((l, i) => (
                          <div key={l.id || i} className="text-[11px] bg-slate-50 rounded px-2 py-1 text-slate-500"><b className="text-slate-600">{l.action}</b> {l.detail} · {l.by} · {new Date(l.ts).toLocaleString('en-IN')}</div>
                        ))}
                      </div>
                    )}
                  </div>
