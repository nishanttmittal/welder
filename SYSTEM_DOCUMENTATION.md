...
| **Any-signed-in-user data exposure** | 🟡 Medium | Anonymous auth is gone (2026-07-23), but **operational** collections (challans, entries, logs) are still read/write for *any* signed-in user — the role rules only protect rates/ledger, payments and users. Fine for a trusted small team; tighten before wider rollout. |
