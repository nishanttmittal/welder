...
|---|---|---|
| **Role enforcement** | ✅ Addressed (welder) | Cloud mode now uses **Google sign-in** for Manager/Owner; roles come from `apps/welder/users` (+ bootstrap owner email). **Firestore rules enforce roles server-side** (transcribed from the live ruleset 2026-07-31, `attendance-app/firestore.rules:107-124`): operational data (dispatches, plating_outbox, logs, meta, parties) = **`wUser()`** — signed in **AND** allowlisted in `apps/welder/users` with `active == true`, **OR** the bootstrap owner email; products/welders/components/receipts = read `wUser`, write `wManager`; rates/ledger/settlements = **manager-read, owner-write** (staff blocked entirely); payments = manager-create / owner edit-reverse; users = owner-write, any signed-in read (needed to resolve a role at login).
⚠️ This row said "operational data = any signed-in" until 2026-07-31. That was wrong, and it is the **second** time a stale line in this row leaked a false security claim into an outside audit. **Read the live ruleset before editing it — do not infer.** **Welders are NOT anonymous** — since 2026-07-23 every user, including each welder, signs in with a personal Google account; the old anonymous `?welder=1&who=Name` link path was removed. Rules are live (published 2026-07-24). |
...
| **Allowlist scope** | 🟢 Low | Anonymous auth is gone (2026-07-23). Operational collections (dispatches, plating_outbox, logs, meta, parties) require **`wUser()`** — signed in **AND** present in `apps/welder/users` **AND** `active == true`, **OR** the bootstrap owner email (`bootstrapOwner()`, the break-glass path that works before any allowlist entry exists) — not merely "any signed-in user". Products/welders/components are read-`wUser`, write-`wManager`. Verified against the live ruleset 2026-07-31 (`attendance-app/firestore.rules:108`). ⚠️ A previous "correction" here (2026-07-28) wrongly said any signed-in user could write; it was written from inference instead of reading the rules. **Read the live ruleset before editing this row.** |
| **Firestore rules live outside this repo** | ✅ | There is **one shared ruleset for all UNICO apps**, and it does **not** live here. Canonical file: `~/attendance-app/firestore.rules`; pull with `node ~/attendance-app/jobs/getRules.js`, publish with `node ~/attendance-app/jobs/deployRules.js` (service account, no Firebase CLI/console needed). The `apps/welder/{collection}/{docId}` rules are already published. The stale copy that used to sit in this repo was deleted 2026-07-28 — never deploy rules from here. |
