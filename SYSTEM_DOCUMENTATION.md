...
|---|---|---|
| **Role enforcement** | ✅ Addressed (welder) | Cloud mode now uses **Google sign-in** for Manager/Owner; roles come from `apps/welder/users` (+ bootstrap owner email). **Firestore rules enforce roles server-side:** operational data = any signed-in; rates/ledger = owner-write; payments = manager-create / owner edit-reverse; users = owner. **Welders are NOT anonymous** — since 2026-07-23 every user, including each welder, signs in with a personal Google account; the old anonymous `?welder=1&who=Name` link path was removed. Rules are live (published 2026-07-24). |
...
| **Allowlist scope** | 🟢 Low | Anonymous auth is gone (2026-07-23). Operational collections (dispatches, plating_outbox, logs, meta, parties) require **`wUser()`** — signed in **AND** present in `apps/welder/users` **AND** `active == true` — not merely "any signed-in user". Products/welders/components are read-`wUser`, write-`wManager`. Verified against the live ruleset 2026-07-30 (`attendance-app/firestore.rules`). ⚠️ A previous "correction" here (2026-07-28) wrongly said any signed-in user could write; it was written from inference instead of reading the rules. **Read the live ruleset before editing this row.** |
